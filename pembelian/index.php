...
$title  = "Transaksi - MyMoro Baby Store";
...
if ($kode) {
    $data = getData("SELECT * FROM tbl_barang WHERE id_barang = '$kode'");
        ...
        <div class="container-fluid">
            <form action="" method="post">
                                ...
                                <div class="col-sm-10">
                                    <select name="kodeBrg" id="kodeBrg" class="form-control">
                                        ...
                                        foreach($barang as $brg) { ?>
                                            <option value="?pilihbrg=<?= $brg['id_barang'] ?>" <?= (isset($_GET['pilihbrg']) && $_GET['pilihbrg'] == $brg['id_barang']) ? 'selected' : '' ?>> <?= $brg['id_barang'] ." | ". $brg['nama_barang'] ?></option>
                                            <?php
                                        }
                                        ?>
                                    </select>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-1">
                            <div class="form-group">
                                ...
                                <input type="number" name="stock" class="form-control form-control-sm" id="stock" value="<?= $selectBrg['stock'] ?? '' ?>" readonly>
                            </div>
                        </div>
                        <div class="col-lg-1">
                            <div class="form-group">
                        ...
                        <div class="col-lg-2">
                            <div class="form-group">
                                ...
                                <input type="number" name="qty" class="form-control form-control-sm" id="qty" value="<?= isset($selectBrg['id_barang']) ? 1 : '' ?>">
                            </div>
                        </div>
                        <div class="col-lg-2">
                            <div class="form-group">
                                <label for="jmlHarga">Total Price</label>
                                <input type="number" name="jmlHarga" class="form-control form-control-sm" id="jmlHarga" value="<?= $selectBrg['harga_beli'] ?? '' ?>" readonly>
                            </div>
                        </div>
                    </div>
                    <button type="submit" class="btn btn-sm btn-info btn-block" name="addbrg"><i class="fas fa-cart-plus fa-sm"></i> Add</button>
                </div>
                            ...
                            foreach ($brgDetail as $detail){ ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="?idbrg=<?= $detail['kode_brg'] ?>&idbeli=<?= $detail['no_beli'] ?>&qty=<?= $detail['qty'] ?>&tgl=<?= $detail['tgl_beli'] ?>&msg=deleted" class="btn btn-sm btn-danger" title="hapus barang" onclick="return confirm('Hapus Barang Ini?')"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                            <?php
                            }
                ...
                <div class="row">
                    ...
                    <div class="col-lg-6 p-2">
                        <button type="submit" name="simpan" id="simpan" class="btn btn-primary btn-sm btn-block"><i class="fa fa-save"></i> Save</button>
                    </div>
                </div>
            </form>
        </div>
